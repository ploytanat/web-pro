    ...
    try {
      ...
      // Check if there is a file upload
        ...
        await conn.query("INSERT INTO images (blog_id, comment_id, file_path, upload_date, main) VALUES (?, ?, ?, CURRENT_TIMESTAMP, 0)",
          [req.params.blogId, result.insertId, file ? file.path.substring(6): ""]
        );
...
router.post('/comments/addlike/:commentId', async function (req, res, next) {
    try {
      ...
      [ req.params.commentId]);
      // ส่งค่า likeNum ที่ถูกต้องกลับไปยัง frontend
      return res.status(200).json({
        blogId: updatedRows[0].blog_id,
        commentId: updatedRows[0].id,
        likeNum: updatedRows[0].like
      });
