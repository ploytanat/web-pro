...
router.get("/blogs/:id", function (req, res, next) {
  ...
  Promise.all([promise1, promise2, promise3])
    .then((results) => {
      const blogs = results[0]; //results[0] คือข้อมูลที่ได้จาก promise1 (blogs)
      const comments = results[1]; //results[1] คือข้อมูลที่ได้จาก promise2 (comments)
      const images = results[2];
      res.render("blogs/detail", {
        blog: blogs[0][0],
        comments: comments[0],
        images: images[0], //ใช้ images[0] เพราะต้องการเรียกข้อมูลรูปอันแรกของblogs เผื่อในกรณีที่blog มีหลายรูป 
        error: null,
      });
    })
    .catch((err) => {
